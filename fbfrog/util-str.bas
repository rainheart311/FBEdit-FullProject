...
function strMatch(byref s as const string, byref szpattern as const string) as integer
   '' Always match?
   if szpattern = "*" then return TRUE

   '' Same? (safe even if the string contains wildcard chars itself)
   if s = szpattern then return TRUE

   '' String <> pattern. String empty?
   if len(s) = 0 then return FALSE

   select case left(szpattern, 1)
   case "*"
      '' Either the rest of the pattern must match right here,
      '' or the pattern must match somewhere later in the string.
      return strMatch(s, right(szpattern, len(szpattern) - 1)) orelse _
             strMatch(right(s, len(s) - 1), szpattern)

   case "?", left(s, 1)
      '' Current char matches; now check the rest.
      return strMatch(right(s, len(s) - 1), right(szpattern, len(szpattern) - 1))
   end select
