...
function hReadableDirExists(byref path as const string) as integer
   dim as string szfixed = path
   if right(szfixed, len(PATHDIV)) = PATHDIV then
      szfixed = left(szfixed, len(szfixed) - len(PATHDIV))
   end if
   function = (len(dir(szfixed, fbDirectory or fbReadOnly or fbHidden)) > 0)
end function
