...
dim shared tokentext(0 to emit.TK__COUNT - 1) as zstring ptr => { _
   @"TK_EOF", _
   @"TK_EOL", _
   @!"\t", _
   @" ", _
...
dim shared as ushort typeToKw(0 to TYPE__COUNT-1) = { _
   TK_TEXT, KW_ANY, _
   KW_BYTE, KW_UBYTE, _
   emit.KW_SHORT, KW_USHORT, _
   emit.KW_LONG, KW_ULONG, _
   KW_CLONG, KW_CULONG, _
   KW_INTEGER, KW_UINTEGER, _
   KW_LONGINT, KW_ULONGINT, _
   emit.KW_SINGLE, emit.KW_DOUBLE, KW_CLONGDOUBLE, _
   TK_TEXT, TK_TEXT, _
...
sub CodeGen.emitExpr(byval n as AstNode ptr, byval need_parens as integer, byval need_macroparam_parens as integer)
   ...
   case ASTKIND_VERNUMCHECK
      add(emit.TK_ID, frog.versiondefine)
      add(TK_SPACE)
      add(emit.TK_EQ)
      add(TK_SPACE)
   ...
   case ASTKIND_TEXT
      add(iif(strIsValidSymbolId(n->text), emit.TK_ID, TK_TEXT), n->text)
   ...
   case ASTKIND_EQ          : emitExpr(n->head, TRUE) : add(TK_SPACE) : add(emit.TK_EQ      ) : add(TK_SPACE) : emitExpr(n->tail, TRUE)
   case ASTKIND_NE          : emitExpr(n->head, TRUE) : add(TK_SPACE) : add(TK_NE      ) : add(TK_SPACE) : emitExpr(n->tail, TRUE)
   case ASTKIND_LT          : emitExpr(n->head, TRUE) : add(TK_SPACE) : add(emit.TK_LT      ) : add(TK_SPACE) : emitExpr(n->tail, TRUE)
   case ASTKIND_LE          : emitExpr(n->head, TRUE) : add(TK_SPACE) : add(TK_LE      ) : add(TK_SPACE) : emitExpr(n->tail, TRUE)
   case ASTKIND_GT          : emitExpr(n->head, TRUE) : add(TK_SPACE) : add(emit.TK_GT      ) : add(TK_SPACE) : emitExpr(n->tail, TRUE)
   ...
   case ASTKIND_ADD         : emitExpr(n->head, TRUE) : add(TK_SPACE) : add(emit.TK_PLUS    ) : add(TK_SPACE) : emitExpr(n->tail, TRUE)
   case ASTKIND_SUB         : emitExpr(n->head, TRUE) : add(TK_SPACE) : add(emit.TK_MINUS   ) : add(TK_SPACE) : emitExpr(n->tail, TRUE)
   case ASTKIND_MUL         : emitExpr(n->head, TRUE) : add(TK_SPACE) : add(emit.TK_STAR    ) : add(TK_SPACE) : emitExpr(n->tail, TRUE)
   case ASTKIND_DIV         : emitExpr(n->head, TRUE) : add(TK_SPACE) : add(emit.TK_SLASH   ) : add(TK_SPACE) : emitExpr(n->tail, TRUE)
   case ASTKIND_MOD         : emitExpr(n->head, TRUE) : add(TK_SPACE) : add(KW_MOD     ) : add(TK_SPACE) : emitExpr(n->tail, TRUE)
   case ASTKIND_INDEX       : emitExpr(n->head, TRUE) :               : add(emit.TK_LBRACKET) :               : emitExpr(n->tail, TRUE) : add(emit.TK_RBRACKET)
   case ASTKIND_MEMBER      : emitExpr(n->head, TRUE) :               : add(emit.TK_DOT     ) :               : emitExpr(n->tail, TRUE)
   case ASTKIND_MEMBERDEREF : emitExpr(n->head, TRUE) :               : add(emit.TK_ARROW   ) :               : emitExpr(n->tail, TRUE)
   case ASTKIND_NOT       : add(KW_NOT  ) : add(TK_SPACE) : emitExpr(n->head, TRUE)
   case ASTKIND_NEGATE    : add(emit.TK_MINUS) : emitExpr(n->head, TRUE)
   case ASTKIND_UNARYPLUS : add(emit.TK_PLUS ) : emitExpr(n->head, TRUE)
   case ASTKIND_ADDROF    : add(TK_AT   ) : emitExpr(n->head, TRUE)
   case ASTKIND_DEREF     : add(emit.TK_STAR ) : emitExpr(n->head, TRUE)
   case ASTKIND_STRINGIFY : add(emit.TK_HASH ) : emitExpr(n->head)
   ...
   case ASTKIND_CAST
      ...
      select case n->dtype
      	case TYPE_BYTE     : add(KW_CBYTE   ) : add(emit.TK_LPAREN)
      	case TYPE_UBYTE    : add(KW_CUBYTE  ) : add(emit.TK_LPAREN)
      case TYPE_SHORT    : add(KW_CSHORT  ) : add(emit.TK_LPAREN)
      case TYPE_USHORT   : add(KW_CUSHORT ) : add(emit.TK_LPAREN)
      case TYPE_LONG     : add(KW_CLNG    ) : add(emit.TK_LPAREN)
      case TYPE_ULONG    : add(KW_CULNG   ) : add(emit.TK_LPAREN)
      case TYPE_INTEGER  : add(KW_CINT    ) : add(emit.TK_LPAREN)
      case TYPE_UINTEGER : add(KW_CUINT   ) : add(emit.TK_LPAREN)
      case TYPE_LONGINT  : add(KW_CLNGINT ) : add(emit.TK_LPAREN)
      case TYPE_ULONGINT : add(KW_CULNGINT) : add(emit.TK_LPAREN)
      case TYPE_SINGLE   : add(KW_CSNG    ) : add(emit.TK_LPAREN)
      case TYPE_DOUBLE   : add(KW_CDBL    ) : add(emit.TK_LPAREN)
      case else
         is_comma_list = TRUE
...
sub CodeGen.emitCode(byval n as AstNode ptr, byval parentkind as integer)
      ...
      if n->expr then
         if n->expr->isCodeScopeBlock() then
            if n->expr->isScopeBlockWith1Stmt() then
               ...
               emitMacroHeader(n, emit.KW_DEFINE)
               eolSingleLineBegin()
               emitCode(n->expr)
               eolSingleLineEnd()
            else
   ...
   case ASTKIND_PPIF
      ...
      case ASTKIND_NOT
         if n->expr->head->kind = ASTKIND_DEFINED then
            add(emit.KW_IFNDEF)
            add(TK_SPACE)
            add(emit.TK_ID, n->expr->head->text)
         end if
      end select

      '' Not handled above? Then emit the default '#if expr'
      if tokens.get(tokens.count - 1) = emit.TK_HASH then
         add(emit.KW_IF)
         add(TK_SPACE)
         emitExpr(n->expr)
      end if
   ...
   case ASTKIND_STRUCT, ASTKIND_UNION, ASTKIND_ENUM
      ...
      '' nesting.
      var opposite = iif(n->kind = ASTKIND_STRUCT, emit.KW_UNION, KW_TYPE)
      ...
      select case n->kind
      case ASTKIND_UNION : compound = emit.KW_UNION
      case ASTKIND_ENUM  : compound = emit.KW_ENUM
      end select
   ...
   case ASTKIND_SELFADD : emitSelfBop(n, emit.TK_PLUS)
   case ASTKIND_SELFSUB : emitSelfBop(n, emit.TK_MINUS)
   case ASTKIND_SELFMUL : emitSelfBop(n, emit.TK_STAR)
   case ASTKIND_SELFDIV : emitSelfBop(n, emit.TK_SLASH)
...
sub TokenRenderer.render(byref tokens as const TokenBuffer)
   ...
   for i as integer = 0 to tokens.count - 1
      ...
      case emit.TK_EOL
         emitLine(ln)
         ln = ""
      case TK_TEXT, emit.TK_ID, TK_STRLIT, TK_NUMLIT
         ln += *tokens.strings.p[tokens.p[i].payload]
      case else
         ln += *tokentext(tk)
      end select
   next
   if (tokens.count > 0) andalso (tokens.get(tokens.count - 1) <> emit.TK_EOL) then
      emitLine(ln)
   end if
end sub
